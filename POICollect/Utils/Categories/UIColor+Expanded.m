...
- (CGFloat)red
{
    NSAssert(self.canProvideRGBComponents, @"必须是RGB的颜色才能使用 red");
...
- (CGFloat)green
{
    NSAssert(self.canProvideRGBComponents, @"必须是RGB的颜色才能使用 green");
    const CGFloat* c = CGColorGetComponents(self.CGColor);
...
- (CGFloat)blue
{
    NSAssert(self.canProvideRGBComponents, @"必须是RGB的颜色才能使用 blue");
    const CGFloat* c = CGColorGetComponents(self.CGColor);
    if (self.colorSpaceModel == kCGColorSpaceModelMonochrome)
        return c[0];
    return c[2];
...
- (CGFloat)white
{
    NSAssert(self.colorSpaceModel == kCGColorSpaceModelMonochrome, @"必须是Monochrome颜色才能使用 white");
    const CGFloat* c = CGColorGetComponents(self.CGColor);
    return c[0];
}
...
- (UInt32)rgbHex
{
    NSAssert(self.canProvideRGBComponents, @"必须是RGB的颜色才能使用rgbHex");
    CGFloat r, g, b, a;
...
{
    NSAssert([self canProvideRGBComponents], @"必须是RGB的颜色才能取到RGB数组");
    CGFloat r, g, b, a;
    if (![self red:&r green:&g blue:&b alpha:&a])
        return nil;

    return [NSArray arrayWithObjects:[NSNumber numberWithFloat:r],
...
{
    return [self colorByAddingRed:f green:f blue:f alpha:0.0];
}
...
{
    NSAssert(self.canProvideRGBComponents, @"必须是RGB颜色才能使用stringFromColor");
    NSString* result;
    switch (self.colorSpaceModel) {
    case kCGColorSpaceModelRGB:
...
+ (UIColor*)colorWithString:(NSString*)stringToConvert
{
    ...
    NSUInteger i = 0;
    if (![scanner scanFloat:(float*)&c[i++]])
        return nil;
    while (1) {
        if ([scanner scanString:@"}" intoString:NULL])
            break;
...
#if SUPPORTS_UNDOCUMENTED_API
@implementation UIColor (Undocumented_Expanded)
